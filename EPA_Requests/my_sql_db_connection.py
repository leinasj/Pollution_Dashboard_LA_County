...
def get_forecast_prediction(zipcode:str, date:str):
    try:
        # Connect to 'air_pollution' MySQL database instance with user credentials 
        cnx = mysql.connector.connect(user='root', password=os.environ.get('MY_SQL_DB_PASS'),
                              host='127.0.0.1',
        ...
        cursor = cnx.cursor()
        # Define SQL query to insert new observations into MySQL database
        add_observation = ("INSERT INTO forecast_data "
                    "(DateIssue, DateForecast, ReportingArea, StateCode, Latitude, Longitude, ParameterName, AQI, AQI_Number, AQI_Classification) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);")
        # API call to airnowapi to get payload response of AQI data from Los Angeles County monitoring sites
        response = requests.get(f'https://www.airnowapi.org/aq/forecast/zipCode/?format=application/json&zipCode={zipcode}&date={date}&distance=10&API_KEY={os.environ.get("AIR_NOW_API_KEY")}')
        # Iterate through JSON response for unique observations, typically monitors will return multiple unique parameters (O3, PM2, PM2.5) 
        for i in response.json():
            observation = (i['DateIssue'], i['DateForecast'], i['ReportingArea'], i['StateCode'], i['Longitude'], i['Latitude'], i['ParameterName'], i['AQI'], i['Category']['Number'], i['Category']['Name'])
            # Upload new observations to database table
            cursor.execute(add_observation, observation)
